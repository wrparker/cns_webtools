...
class LoginController extends Controller
{
    ...
    */
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::attempt(['username' => $request->input('username'), 'password' => $request->input('password'), 'enabled' => true])) {
            // Authentication passed...
            return redirect()->intended($this->redirectTo);
        }

        //#2 Lets see if LDAP is enabled.  Controlled with globals in web.php
       else if(AUTH_LDAP_ENABLED === true){
           ...
           else if(sizeof($local_lookup = \App\User::where('username', trim($request->input('username')))->get()) === 1){
               if (LDAP_authenticator::LDAPAuthenticateEIDPassword(trim($request->input('username')), $request->input('password'))) {
